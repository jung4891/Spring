...
public class HomeController {
	...
	public String createTable(Locale locale, Model model) {
		DBCommon<Student> db = new DBCommon<Student>("c:/tomcat/0521.db", "student");
	...
	public String insertAct(@RequestParam("student_name") String name,
			@RequestParam("mid") String midStr, @RequestParam("fin") String finStr, Model model) {
		int mid = Integer.parseInt(midStr);  // 서버는 String으로 값을 받아온다. 그래야 오류가 안난다고 하심
		int fin = Integer.parseInt(finStr);  
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String now = sdf.format(Calendar.getInstance().getTime());
		DBCommon<Student> db = new DBCommon<Student>("c:/tomcat/0521.db", "student");
		db.insertData(new Student(name, mid, fin, now));
	...
	public String create(Locale locale, Model model) {  
		...
		for (int i=0; i<students.size(); i++) {
			...
			htmlString += "</td>";
			htmlString += "<td>";
			htmlString += students.get(i).mid;
			htmlString += "</td>";
			htmlString += "<td>";
			htmlString += students.get(i).fin;
			htmlString += "</td>";
			htmlString += "<td>";
